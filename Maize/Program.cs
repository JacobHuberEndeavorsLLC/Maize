...
Console.Clear();
font.ToPrimary("Growing...");
IConfiguration config = new ConfigurationBuilder()
    .AddJsonFile(appSettingsEnvironment)
    .AddEnvironmentVariables()
    .Build();
Settings s = config.GetRequiredSection("Settings").Get<Settings>();
var environment = Constants.GetNetworkConfig(s.Environment);
ApplicationUtilities.EnvironmentCheck(font, s.LoopringAccountId, s.LoopringApiKey, net, appSettingsEnvironment);

ILoopringService loopringService = new LoopringService(environment.Url, font);
var signedMessage = EDDSAHelper.EddsaSignUrl(s.LoopringPrivateKey, HttpMethod.Get, new List<(string Key, string Value)>() { ("accountId", s.LoopringAccountId.ToString()) }, null, "api/v3/apiKey", environment.Url);
var apiKey = await loopringService.GetApiKey(s.LoopringAccountId, signedMessage);
...
font = await ApplicationUtilities.NftChecks(s, loopringService, font);
loopringService = new LoopringService(environment.Url, font);

Menu.BannerForMaize(font);
//Menu.BannerForMaize(font, await ApplicationUtilities.LeaderBoardForMenu(s, loopringService, environment));

//font.ToTertiary("Ready to start?");
var userResponseReadyToMoveOn = ApplicationUtilities.CheckYesOrNo(font, "Ready to start");

var ens = await loopringService.GetLoopringEns(s.LoopringApiKey, s.LoopringAddress);
while (userResponseReadyToMoveOn == "yes" || userResponseReadyToMoveOn == "y")
{
    Console.WriteLine();
    font.SetVersionFontColor($"Hey,", $" {((!string.IsNullOrEmpty(ens.data)) ? ens.data : s.LoopringAddress)}.", " I hope you're doing corntastic <3");
    var menuAndUtility = Menu.MenuForMaize(font, s.Environment);
    ...
    switch (menuAndUtility.userResponseOnUtility)
    {
        ...
        case 0:
            ...
            font.ToWhite($"\t The transaction fees are {Constants.LcrTransactionFee} {Constants.MaxFeeToken} per transfer.");
            font.ToWhite($"\t This includes both Nft and {Constants.MaxFeeToken} transfers.");
        ...
        #region case 1
        case 1:
            Menu.UtilityHeader(font, menuAndUtility.allUtilities.ElementAt(1).Value,
                "Here you will get the NFT Data of the NFTs in a wallet.",
                "You will need a wallet address or ENS.", 
                "", false);

            var nftDataFromWallet = await ApplicationFunctionalities.NftDataFromWallet(font, loopringService, s);

            ApplicationUtilities.FunctionalityProcessTime(nftDataFromWallet.sw, nftDataFromWallet.fileName, null, null, font);
        break;
        #endregion case 1
        #region case 2
        case 2:
            Menu.UtilityHeader(font, menuAndUtility.allUtilities.ElementAt(2).Value,
                "Here you will get NFT Data for the NFTs in a Collection.",
                "You will need the collection address.",
                "You might need the Minter address and NFT Id", false);

            var nftDataFromCollection = await ApplicationFunctionalities.NftDataFromCollection(font, loopringService, s);

            ApplicationUtilities.FunctionalityProcessTime(nftDataFromCollection.sw, nftDataFromCollection.fileName, null, null, font);
            break;
        ...
        case 3:
            var utility = 3;
            Menu.UtilityHeader(font, menuAndUtility.allUtilities.ElementAt(utility).Value,
            "Here you will get wallet addresses that hold specific Nfts.",
            "You will need the Nft Data.",
            "", false);

            var nftHoldersFromNftData = await ApplicationFunctionalities.NftHoldersFromNftData(font, loopringService, nftMetadataService, ethereumService, s, utility);

            ApplicationUtilities.FunctionalityProcessTime(nftHoldersFromNftData.sw, nftHoldersFromNftData.fileName, nftHoldersFromNftData.fileNameTwo, null, font);
