...
namespace LoopMintSharp
{
    public class Minter
    {
        static ILoopringMintService loopringMintService;
        static int chainId;

        public Minter(string url, int chainId)
        {
            Minter.chainId = chainId;
            loopringMintService = new LoopringMintService(url);
        }
    ...
    string baseUri
    )
        {
            ...
            var mintFee = await loopringMintService.GetOffChainFee(loopringApiKey, accountId, 9, counterFactualNft.tokenAddress);
            return mintFee;
        }
        ...
        {
            //Getting the token address
            var mintFee = await loopringMintService.GetOffChainFeeWithAmount(loopringApiKey, accountId, 0, 11, tokenAddress);
            if (verboseLogging)
            {
            ...
            bool verboseLogging)
        {
            ...
            var collectionResult = await loopringMintService.CreateNftCollection(apiKey, createCollectionRequest, sha256Signed);
        ...
        public async Task<CollectionResult> FindNftCollection(string loopringApiKey, int limit, int offset, string owner, string contractAddress)
        {
        ...
        public async Task<(MintResponseData, decimal)> MintCollection(string loopringApiKey,
                       string loopringPrivateKey,
                       string? minterAddress,
                       int accountId,
                       int nftType,
                       ...
                       string royaltyAddress)
        {
            ...
            //Getting the offchain fee
            var offChainFee = await loopringMintService.GetOffChainFee(loopringApiKey, accountId, 9, tokenAddress);
            #endregion
        ...
        {
            return await loopringMintService.GetTokenIdWithCheck(loopringApiKey, loopringAccountId, nftData);
        }
                 ...
                 bool verboseLogging)
        {
            ...
            var offchainFee = await loopringMintService.GetOffChainFeeWithAmount(loopringApiKey, accountId, 0, 11, nftBalance.data[0].tokenAddress);
            var storageId = await loopringMintService.GetNextStorageId(loopringApiKey, accountId, nftBalance.data[0].tokenId);
            ...
            BigInteger[] poseidonInputs =
    {
            ...
            eip712TypedData.Domain = new Domain()
            {
                ...
                Version = "3.6.0",
                ChainId = chainId,
                VerifyingContract = "0x12b7cccF30ba360e5041C6Ce239C9a188B709b2B",
            };
            ...
            var mintResponse = await loopringMintService.MintRedPacketNft(loopringApiKey, ecdsaSignature, redPacketNft);
